...
const Airtime = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!value || value <= 0) {
      setMessage({ type: 'error', text: 'Choose an airtime amount.' });
      return;
    }
    if (!currentUser || value > currentUser.balance) {
      setMessage({ type: 'error', text: 'Insufficient balance for airtime purchase.' });
      return;
    }
    ...
    setAmount('');
    refreshUser();
    setMessage({ type: 'success', text: 'Airtime purchase completed.' });
  };
        ...
        <main className="space-y-6">
          <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-2xl shadow-slate-950/40">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-cyan-300/80">Airtime</p>
                <h1 className="text-3xl font-semibold text-white">Buy Airtime</h1>
              </div>
              <p className="text-sm text-slate-400">Quick phone top-up using your USDC balance.</p>
            </div>
          </div>
