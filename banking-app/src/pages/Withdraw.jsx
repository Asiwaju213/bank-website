...
const Withdraw = () => {
  ...
  const handleSubmit = (e) => {
    ...
    addTransaction({
      ...
      description: `Withdrew ${value.toFixed(2)} USDC`,
    });
        ...
        <main className="space-y-6">
          <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-2xl shadow-slate-950/40">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-cyan-300/80">Withdraw</p>
                <h1 className="text-3xl font-semibold text-white">Cash out your funds</h1>
              </div>
              <p className="text-sm text-slate-400">Withdraw funds from your account.</p>
            </div>
          </div>
          ...
          <section className="rounded-3xl border border-white/10 bg-slate-950/90 p-8 shadow-2xl shadow-slate-950/40">
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div>
                <label className="text-sm text-slate-300">Withdraw Amount</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
